...
import (
	...
	x "github.com/emmansun/gmsm/smx509"
	"io/ioutil"
)
...
func convertBag(bag *safeBag, password []byte) (*pem.Block, error) {
	...
	case bag.Id.Equal(oidPKCS8ShroundedKeyBag):
		...
		case *ecdsa.PrivateKey:
			block.Bytes, err = x.MarshalECPrivateKey(key)
			if err != nil {
				return nil, err
			}
		case *sm2.PrivateKey:
			block.Bytes, err = x.MarshalSM2PrivateKey(key)
			if err != nil {
				return nil, err
			}
		default:
...
// pfxData.
func Decode(pfxData []byte, password string) (privateKey interface{}, certificate *x.Certificate, err error) {
	encodedPassword, err := bmpString(password)
	if err != nil {
		return nil, nil, err
	}
...
func Encode(privateKey interface{}, certificate *x.Certificate, caCerts []*x.Certificate, password string) (pfxData []byte, err error) {
	encodedPassword, err := bmpString(password)
	if err != nil {
		return nil, err
	}
